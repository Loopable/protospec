...
Test vectors for signatures and encryption require freezing every byte of the wire format and key material, which was premature while `decisions/0004` and `0009` are still open. Separately, the three blocking constructions in `decisions/0011` to `0013` changed bytes that had no vectors at all, leaving their behaviour unverifiable.
...
`test-vectors/` contains byte-exact vectors for first-device authorization, HPKE object-key wrapping, and versioned-object AAD, plus a reproducible generator (`test-vectors/generate.mjs`). The remaining vectors in the coverage plan are generated when `0004` and `0009` resolve and the wire format freezes, before 1.0.
...
* The rest of the coverage plan remains a contract on the critical path for 1.0.
